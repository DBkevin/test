...
For the current Douyin flow, if the page already shows a selected `团购` tab together with the inline group-buy keyword entry, treat it as a usable group-buy state and do not tap `团购` again just to force a stricter classifier match.
For Douyin merchant-result confirmation, do not rely on the current activity name alone or on one fixed vertical band for the merchant title. Prefer the combined page structure: top result tabs such as `团购 / 直播 / 视频`, the target merchant node, and merchant-result context such as `评价` / `回头客` / `人均` / distance.
When a real-device debugging round produces a new stable conclusion, write it back into the runbook and add a dated handoff note under `docs/` so the conclusion survives context compression.
